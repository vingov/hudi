...
public class HoodieBigQueryClient extends AbstractSyncHoodieClient {
  ...
  public HoodieBigQueryClient(final BigQuerySyncConfig syncConfig, final FileSystem fs) {
    super(syncConfig.basePath, syncConfig.assumeDatePartitioning, syncConfig.useFileListingFromMetadata,
        false, fs);
    this.createBigQueryConnection(syncConfig.projectId);
  }

  private void createBigQueryConnection(String projectId) {
    if (bigquery == null) {
      try {
  ...
  public void createManifestTable(
      String projectId, String datasetName, String tableName, String manifestSourceUri) {
    try {
      TableId tableId = TableId.of(projectId, datasetName, tableName);
      ...
      ExternalTableDefinition customTable =
          ExternalTableDefinition.newBuilder(manifestSourceUri, schema, csvOptions)
              .setAutodetect(false)
  ...
  public void createSnapshotView(
    ...
    try {
      ...
      String query =
          String.format(
              "SELECT * FROM `%s.%s.%s` WHERE _hoodie_file_name IN (SELECT filename FROM `%s.%s.%s`)",
              projectId,
              datasetName,
              versionsTableName,
              projectId,
      ...
      LOG.info("Snapshot view created successfully");
    } catch (BigQueryException e) {
      throw new HoodieBigQuerySyncException("Snapshot view was not created ", e);
    }
  }
